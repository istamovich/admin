...
const Discount = () => {
    ...
    const fetchDiscounts = async () => {
        try {
            ...
            setDiscounts(response.data.data); // Assign the fetched discounts to state
        } catch (error) {
                            ...
                            <tbody>
                                {discounts.map((discount, index) => (
                                    <tr key={discount.id} className="text-center hover:bg-gray-100">
                                        ...
                                        <td className="border border-gray-300 p-2">{discount.discount}</td>
                                        <td className="border border-gray-300 p-2">{discount.started_at}</td>
                                        <td className="border border-gray-300 p-2">{discount.finished_at}</td>
                                        <td className="border border-gray-300 p-2">
                                            <span
                                                className={`px-3 py-1 rounded-full text-white ${discount.status ? 'bg-green-500' : 'bg-red-500'}`}
                                            >
